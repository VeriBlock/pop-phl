dnl Copyright (c) 2013-2016 The Placeholders Core developers
...
AC_DEFUN([PHL_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build placeh-qt GUI (default=auto)])],
    [
...
AC_DEFUN([PHL_QT_CONFIGURE],[
  ...
  if test x$placeh_enable_qt = xyes; then
    AC_MSG_RESULT([$placeh_enable_qt ($QT_LIB_PREFIX)])
  else
    AC_MSG_RESULT([$placeh_enable_qt])
  fi
...
AC_DEFUN([_PHL_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, placeh_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
