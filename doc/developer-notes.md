...
- [Development guidelines](#development-guidelines)
    - [General Placeholders Core](#general-placeh-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Placeholders Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Placeholders Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/placeh/placeh/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_placeh
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_placeh --log_level=test_suite
$ valgrind -v --leak-check=full src/placehd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_placeh.coverage/index.html`.
```
...
To profile a running placehd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep placehd` -- sleep 60
```
...
Placeholders Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/placeh/placeh/issues/12691)
...
- ThreadRPCServer : Remote procedure call handler, listens on port 6608 for connections and services them.
...
However, in open source software such as Placeholders Core, where everyone uses
...
pay attention to for reviewers of Placeholders Core code.

General Placeholders Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Placeholders Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `PLACEH_FOO_BAR_H`, e.g.

```c++
#ifndef PLACEH_FOO_BAR_H
#define PLACEH_FOO_BAR_H
...
#endif // PLACEH_FOO_BAR_H
```
...
Some of these are maintained by active developers of Placeholders Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Placeholders Core so that they can be integrated
...
- src/secp256k1
  - Upstream at https://github.com/placeh-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/placeh-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/placeh-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with Placeholders's `select()` loop, because
...
compatibility between nodes. This happened in Placeholders 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Placeholders's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/placeh/placeh/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/placeh/placeh/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/placeh/placeh/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Placeholders Core, the wallet was always
