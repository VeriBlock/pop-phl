Placeholders Core version 0.13.0 is now available from:

  <https://placehproject.com/bin/placeh-core-0.13.0/>
...
Various code modernizations have been done. The Placeholders Core code base has
...
HD wallets are incompatible with older versions of Placeholders Core.
...
proposed parameters, a future release of Placeholders Core is expected that
...
The two phases are distinct in the Placeholders-Qt GUI. During the first one,
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use PLACEH/kB (MarcoFalke)
...
- #8207 `f7a403b` Add a link to the Placeholders-Core repository and website to the About Dialog (MarcoFalke)
...
- #8164 `0f24eaf` [Placeholders-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
