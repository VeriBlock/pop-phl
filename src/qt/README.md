This directory contains the PlaceholdersQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
ClientModel is used by the main application `placehgui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a placeh:... link from a browser).
...
* `placehunits.(h/cpp)`: PLACEH / mPLACEH / etc handling
* `callback.h`
...
* don't change `local/placeh_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "placeh-qt" under "Run", which is where you can also add command line arguments)
