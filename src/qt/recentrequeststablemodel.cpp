    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return PlaceholdersUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, PlaceholdersUnits::separatorNever);
            else
                return PlaceholdersUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+PlaceholdersUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
