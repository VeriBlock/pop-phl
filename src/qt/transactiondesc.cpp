...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord *rec, int unit)
{
    ...
    try { 
        ...
        if(isAttackInProgress == true ) { 
            vbkMessage = "Alternate Chain Detected, wait for Placeholders Finality";
        }
        else {
            if( !err ) { 
                if( spFinality > 0 ) { 
                    vbkMessage = "" + QString::number(spFinality) + tr(" blocks of Placeholders Finality") ;
                } else if( spFinality <= 0 ) { 
                    vbkMessage = "" + QString::number(spFinality) + tr(" blocks until Placeholders Finality") ;
                }
            }
        }
    } catch(...) { 
