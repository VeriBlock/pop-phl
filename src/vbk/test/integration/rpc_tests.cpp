...
BOOST_AUTO_TEST_CASE(getpopdata_test)
{
    ...
    BOOST_CHECK(find_value(result.get_obj(), "last_known_placeh_blocks").get_array().size() == 15); // number of bootsrap blocks that have been set up in .properties file
}
...
BOOST_AUTO_TEST_CASE(updatecontext_test)
{
    ...
    UniValue placeh_blocks(UniValue::VARR);
    placeh_blocks.push_back("00000020E2EEC323DA41F7F3387E8E4F1BF313EDFD2247788CBA378900040000000000002878DE94F86A20B384C0B0FC6BAAD61EBB0DCDC7126684072CCAD3EE624905060F60FB5DFFFF001B2CBAA52B");

    request.params.push_back(placeh_blocks);
    ...
    BOOST_CHECK(find_value(result.get_obj(), "last_known_placeh_blocks").get_array().size() == 16); // number of bustrap blocks that have been set up in .properties file
}
